...
def init(update, context):
    ...
    if row is not None:
        message = "{} il tuo <b>punteggio</b> nella chat {} è di <code>{}</code> punti".format(update.effective_user.first_name,update.message.chat.title,row[0])
        update.effective_message.reply_html(message)
    else:
        update.effective_message.reply_html("ERRORE")